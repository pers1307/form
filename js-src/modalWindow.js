function ModalWindow() {
     ...
     * @type {string}
     */
    this.clickClass = '.js-order';

    /**
     * Класс в котором хранится модалка
     * @type {string}
     */
    this.classWithModal = '.js-noDisplay';

    /**
     * Постфикс в котором хранится модалка
     * @type {string}
     */
    this.withModalPostFix = '';
    ...
    /**
     * Инициализвция событий формы
     */
    this.initModalEvent = function () {
         ...
         */
        $(document).on('click', this.clickClass, function(e){
            e.preventDefault();
    ...
    this.intUploader = function () {
        ...
        $(".modal-window-wrap " + this.uploadItem).attr('id', 'browse');
        // Потом дергаем этот элемент из DOM
        var elem1 = document.getElementById("browse");

        var fileUrl = $(".modal-window-wrap " + this.form + this.formPostFix).attr('data-file');
        ...
        this.uploader.bind('FilesAdded', function(up, files) {
            ...
            if (self.fileFirst == false) {
                ...
                $(".modalContent " + self.uploadList).append(html);
                self.uploader.start();
                self.fileFirst = true;
            } else {
        ...
        this.uploader.bind('UploadProgress', function(up, file) {
            ...
            $(".modal-window-wrap " + self.form + self.formPostFix).addClass('loading');
        });
        ...
        this.uploader.bind('FileUploaded', function(up, file, info) {
            ...
            $(".modal-window-wrap " + self.form + self.formPostFix).removeClass('loading');

            if (response.data.error == 'maxSizeFile') {
                self.resetErrorClass($(self.fileUploadArea));

                $(".modalContent " + self.uploadList).empty();
    ...
    this.deleteFileAction = function () {
        ...
        var deleteFileUrl = $(".modal-window-wrap " + this.form + this.formPostFix).attr('data-deleteFile');
